...
import {
  Search, X, Filter, Loader2, ChevronLeft, Heart, Users, GraduationCap, Briefcase, Rocket, MapPin, TrendingUp, Sparkles,
} from "lucide-react";
...
const INTEREST_SUGGESTIONS = [
  "Coding", "Hackathons", "Coffee", "Anime", "Gaming", "Music", "Reading", "Sports",
  "Photography", "Travel", "Cooking", "Movies", "Startup", "AI/ML", "Web Dev", "Design",
];
...
function SearchPage() {
      ...
      <div className="mx-auto max-w-4xl space-y-6 p-6 pb-28 md:pb-8">
        <div className="animate-fade-up flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">Find people by name, interests, branch, and more</p>
          </div>
        </div>
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 p-6 pb-28 md:pb-8">
      ...
      {/* Search Bar */}
      <form onSubmit={handleSearch} className="rounded-2xl border border-border glass p-4 animate-fade-up">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {RELATIONSHIP_OPTIONS.map(opt => (
                    <label key={opt.value} className="inline-flex items-center gap-2 rounded-xl border px-3 py-1.5 text-xs cursor-pointer transition hover:border-primary hover:bg-surface-elevated">
                      <input
                        type="checkbox"
        ...
        {isError ? (
          ...
          profiles.map((profile) => (
            <ProfileCard
              key={profile.id}
              profile={profile}
              onLike={profileId => { /* handled by discovery tabs */ }}
              onPass={profileId => { }}
              onSave={profileId => { }}
