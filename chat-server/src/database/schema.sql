...
-- Seed initial profiles for testing (Idempotent: prevents duplicate records on re-runs)
INSERT INTO dating_profiles (name, age, year, major, bio, interests, emoji, verified)
SELECT 'Anjali Sharma', 20, '3rd Year', 'Design', 'Always sketchin'' in class. Coffee lover, indie music fan, and looking for someone to review campus cafes with!', ARRAY['Art', 'Indie Rock', 'Cafes', 'UI/UX'], '🎨', true
WHERE NOT EXISTS (SELECT 1 FROM dating_profiles WHERE name = 'Anjali Sharma');

INSERT INTO dating_profiles (name, age, year, major, bio, interests, emoji, verified)
SELECT 'Vikram Sen', 21, '4th Year', 'Mechanical Eng.', 'Car enthusiast, amateur guitar player, and gym regular. Let''s study (or skip lectures) together.', ARRAY['Gym', 'Guitars', 'Anime', 'Formula 1'], '🎸', false
WHERE NOT EXISTS (SELECT 1 FROM dating_profiles WHERE name = 'Vikram Sen');

INSERT INTO dating_profiles (name, age, year, major, bio, interests, emoji, verified)
SELECT 'Kavya Iyer', 19, '2nd Year', 'Economics', 'If you love debate, board games, and late night chai, we will probably get along. Bookworm 📚', ARRAY['Chai', 'Debating', 'Chess', 'Reading'], '♟️', true
WHERE NOT EXISTS (SELECT 1 FROM dating_profiles WHERE name = 'Kavya Iyer');

INSERT INTO dating_profiles (name, age, year, major, bio, interests, emoji, verified)
SELECT 'Rohan Varma', 20, '3rd Year', 'Computer Science', 'I build websites and compile errors for fun. Let''s match if you want someone to debug your life.', ARRAY['Coding', 'Hackathons', 'Valorant', 'Memes'], '💻', true
WHERE NOT EXISTS (SELECT 1 FROM dating_profiles WHERE name = 'Rohan Varma');

INSERT INTO dating_profiles (name, age, year, major, bio, interests, emoji, verified)
SELECT 'Tanya Kapoor', 20, '3rd Year', 'English Lit.', 'Poetry, street photography, and vintage vinyl records are my jam. Tell me your favorite movie?', ARRAY['Poetry', 'Cinema', 'Vinyls', 'Travel'], '📷', false
WHERE NOT EXISTS (SELECT 1 FROM dating_profiles WHERE name = 'Tanya Kapoor');

INSERT INTO dating_profiles (name, age, year, major, bio, interests, emoji, verified)
SELECT 'Siddharth M.', 20, '3rd Year', 'Computer Science', 'Building campus apps for fun. Hackathon enthusiast. Ask me about my startup idea.', ARRAY['Coding', 'Startups', 'Chess', 'Coffee'], '🚀', true
WHERE NOT EXISTS (SELECT 1 FROM dating_profiles WHERE name = 'Siddharth M.');
